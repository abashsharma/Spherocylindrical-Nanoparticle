...
public:
    ...
    std::vector<std::vector<bool>> PrintEdgeMap();
    std::vector<Triangle<T>> PrintSphereTriangles();
...
template<typename T>
std::vector<std::vector<bool>> tesselaSphere<T>::PrintEdgeMap()
{
    return EdgeMap;
}

template<typename T>
std::vector<Triangle<T>> tesselaSphere<T>::PrintSphereTriangles()
{
